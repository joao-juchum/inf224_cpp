...
// Testing MediaManager 02 (Load and Save)
void testMediaManager2() {
    MediaManager manager;

    // Criar e adicionar alguns objetos multimídia ao manager
    auto photo = manager.createPhoto("banana", "./images_inf224/banana.jpeg", 48.8584, 2.2945);
    auto video = manager.createVideo("video1", "./videos_inf224/Video_cat_1.mp4", 120);
    auto film = manager.createFilm("film1", "./videos_inf224/Video_cat_2.mp4", 150, 3, new int[3]{10, 20, 30});

    // Criar um grupo e adicionar os objetos multimídia a ele
    auto mediaGroup = manager.createGroup("Media Group");
    mediaGroup->push_back(photo);
    mediaGroup->push_back(video);
    mediaGroup->push_back(film);

    // Salvar os objetos em um arquivo
    std::string filename = "media_data.txt";
    manager.save(filename);
    std::cout << "\nObjetos salvos em " << filename << std::endl;

    // // Limpar o `MediaManager` e recarregar os objetos do arquivo para testar a desserialização
    // manager = MediaManager(); // Isso recria o gerenciador, limpando as tabelas

    // // Carregar os objetos a partir do arquivo
    // manager.load(filename);
    // std::cout << "\nObjetos carregados de " << filename << std::endl;

    // // Exibir os objetos carregados
